// Get the page loading speed with chromium and edge caching AND prioritizing
...
const testSite_cached = [
    "https://192.168.1.167:8081/fe82dac84d8b896b3e7cf9edcb4f0eb8"
];
...
async function evaluate_indeNetwork(currentNetworkCondition) {
    for(let i = 0; i < testSite_original.length; i++) {
        console.log("Benchmarking ", testSite_original[i]);
        let testSite = new URL(testSite_original[i]);
        let testResultName = testSite.hostname + "_" + currentNetworkCondition + "_reprioritizing" + ".csv"; // benchmark result csv file
        await fs.writeFile(testResultName, "Test Site, Reprioritized Load Time\n");
        console.log("Network Status:",currentNetworkCondition);
        // console.log(networkPreset);
        for(let j = 0; j < BENCHMARK_TIMES; j++) {
            console.log(`Test No.${j+1}`);
            // let originalLoadTime = await getDOMLoadedTime(testSite_original[i]);
            let reprioritizedTime = await getDOMLoadedTime(testSite_cached[i]);
            console.log("Reprioritized: ",reprioritizedTime);
            await fs.appendFile(testResultName, `${testSite_original[i]},${reprioritizedTime}\n`);
        }
    }
}
evaluate_indeNetwork("ex_crowded");
